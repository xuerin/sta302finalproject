...
packages <- c("tidyverse", "latex2exp", "gridExtra", "rvest", "kableExtra", "leaps", "ggplot2", "patchwork")
...
# Residual plots
```{r, fig.width=12, fig.height=8,message=FALSE, echo=FALSE, warning=FALSE}
par(mfrow = c(2, 2))
plot(model)
```
# Check for regression assumptions. Residual VS each predictor
```{r, fig.width=12, fig.height=8, message=FALSE, echo=FALSE,warning=FALSE}
...
hist_plot <- ggplot(all_data, aes(x = residuals)) +
  geom_histogram(binwidth = 500, fill = "steelblue", color = "white", alpha = 0.7) +
