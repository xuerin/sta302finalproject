...
The Q-Q plot showed that residuals were approximately normal, with minor deviation in the lower tail. Residuals versus individual predictors showed random scatter for income composition and schooling, but some structure for expenditure, population, and polio, suggesting potential nonlinearities.  

Overall, the model shows strong evidence that educational and economic factors influence GDP, though future improvements could explore nonlinear modeling or methods to address residual heteroscedasticity.  
Figure 4: More graphs for regression assumptions
