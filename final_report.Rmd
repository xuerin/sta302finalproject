...
packages <- c("tidyverse", "latex2exp", "gridExtra", "rvest", 
              "kableExtra", "leaps", "ggplot2", "patchwork", "MASS")
...
# Fit a model with original GDP and desired predictors (same as model3)
temp_model <- lm(GDP ~ Status + Total.expenditure + Polio + Population +
                 Income.composition.of.resources + Schooling)
# Box-Cox transformation(λ=0.1010101)
bc <- boxcox(temp_model, plotit = TRUE)
(lambda_optimal <- bc$x[which.max(bc$y)])

all_data$bc_GDP <- (all_data$GDP^lambda_optimal - 1) / lambda_optimal
...
model5 <- lm(bc_GDP ~ Status + Total.expenditure +Polio
             + I(Population^2)
