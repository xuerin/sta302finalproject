...
&= \hat{\beta_0} + hat{\beta_1} X_{\text{Status}} + \hat{\beta_2} X_{\text{TotalExpenditure}}^2 + \hat{\beta_3} X_{\text{Polio}} \\
&\quad + \hat{\beta_4} \log(X_{\text{Population}}) + \hat{\beta_5} X_{\text{IncomeCompositionofResources}} + \hat{\beta_6} X_{\text{Schooling}}^2 + \varepsilon
\end{align*}
...
### Population (log):
The coefficient for log(Population) is –0.0022 and not statistically significant (p = 0.930). This suggests population size does not explain additional variation in GDP after accounting for other factors. The 95% confidence interval $[–0.0513, 0.0468]$ includes 0, confirming no significant relationship at the 5% level.
