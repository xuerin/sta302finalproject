...
# Box-Cox transformation(λ=0.1010101)
...
# Model 5 (box-cox & polynomial)

model5 <- lm(bc_GDP ~ Status +I(Total.expenditure^2) +Polio
             + I(log(Population))
...
# AIC & BIC
aic_results <- AIC(model1, model2, model3, model4, model5)
bic_results <- BIC(model1, model2, model3, model4, model5)
print(aic_results)
print(bic_results)

#check model 5
mod.resid <- resid(model5)
mod.fitted <- fitted(model5)
mod.sresid <- rstandard(model5)
...
clean_data5 <- all_data[-remove_invalid, ]

# clean model

clean_model5 <- lm(bc_GDP ~ Status 
